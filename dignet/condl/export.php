...
$strSql = "SELECT * FROM t_pubmed_query where c_project_id != 'Cardiotoxicity'";
...
foreach ($rs as $row) {
	...
	if ($pubmedRecords=='') {
		...
		file_put_contents("exportFiles/$keywords.p.scores", join(PHP_EOL, $arrayScore['p']));
		file_put_contents("exportFiles/$keywords.d.scores", join(PHP_EOL, $arrayScore['d']));
		file_put_contents("exportFiles/$keywords.b.scores", join(PHP_EOL, $arrayScore['b']));
		file_put_contents("exportFiles/$keywords.c.scores", join(PHP_EOL, $arrayScore['c']));
	}
}
