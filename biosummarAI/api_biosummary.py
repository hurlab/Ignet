...
from cheroot import wsgi # Use Cheroot server
...
urls = (
    '/biobert/', 'biobert' # Endpoint remains the same
)
...
def estimate_token_count(text):
    if not tokenizer: return len(text.split()) # Fallback estimation
...
def split_text(text, token_limit):
    if not tokenizer: # Fallback splitting if tokenizer failed
        words = text.split()
        chunks = []
        current_chunk_words = []
        ...
        for word in words:
             ...
             # Approximate chunk size based on average token/word ratio
    ...
    for token in tokens:
        ...
        # Check size slightly before the limit to be safe
...
def get_initial_summary_with_gpt4o(data_input, instructions=""):
    ...
    safety_margin = 5000 # Increased margin for safety, prompt, response
    token_limit = model_context_limit - safety_margin
    max_summary_tokens = 400 # Target for ~250 words

    combined_text = ""
    # Process DataFrame input
    if isinstance(data_input, pd.DataFrame):
        # Convert DataFrame rows to text, handle potential NA values
    ...
    summaries = []
    # ** Updated Prompt with instructions from user **
    ...
    initial_prompt_template = (
        ...
        "Data:\n"
        "'{chunk_text}'"
        "\n\nSummary:" # Add separator for clarity
    )

    # --- Chunking Logic ---
    text_to_process = combined_text
    # If input exceeds limit, split into chunks.
    if input_token_count > token_limit:
        ...
        # If chunking results in too many chunks (e.g., > 10), maybe just take the first few
        ...
        for i, chunk in enumerate(text_chunks):
            ...
            try:
                # Use slightly fewer max tokens per chunk summary
                response = openai.chat.completions.create(
                    model="gpt-4o",
                    messages=[{"role": "user", "content": prompt}],
                    temperature=0.2,
                    max_tokens=max_summary_tokens // len(text_chunks) if len(text_chunks) > 0 else max_summary_tokens, # Divide tokens among chunks
                    timeout=60 # Timeout for individual chunk API call
                )
        ...
        if len(summaries) > 1:
            ...
            try:
                 final_response = openai.chat.completions.create(
                    ...
                    temperature=0.1,
                    max_tokens=max_summary_tokens + 100, # Allow slightly more for combining
                    timeout=60
                 )
                 final_summary = final_response.choices[0].message.content
                 text_to_process = final_summary # Use combined summary as the final result
            except Exception as e:
    ...
    # --- Single Pass Summary (if input was small enough or after combining chunks) ---
    else: # Input fits within limit, process directly
        ...
        try:
            response = openai.chat.completions.create(
                ...
                messages=[{"role": "user", "content": prompt}],
                temperature=0.2,
                max_tokens=max_summary_tokens + 50, # Allow slightly more if single pass
                timeout=90 # Longer timeout for potentially large single request
            )
...
def chat_with_openai_cont(conv, prompt):
    ...
    # --- MODIFICATION START: Add length constraint and change max_tokens ---
    # Add length constraint to the user's prompt
    modified_prompt = f"{prompt}\n\n(Please keep your response concise, around 50 words.)"
    
    # Append the user's *modified* message
    ...
    try:
        response = openai.chat.completions.create(
            ...
            messages=conv,
            temperature=0.5,
            max_tokens=100,  # Hard limit (approx 75 words) to enforce brevity
            timeout=60 # Timeout for chat response
        )
    # --- MODIFICATION END ---
        assistant_reply = response.choices[0].message.content
        # Append the assistant's reply to the history for the *next* turn
        conv.append({"role": "assistant", "content": assistant_reply})
        console.print(Markdown(f"**BioSummarAI:** {assistant_reply}"))
        # Return the latest reply and the updated full history
        ...
        # Remove the user message that caused the error to avoid loop
...
class biobert:
    def POST(self):
        global api_key # Allow modification if needed
        if not api_key:
             # Try reloading from .env just in case
             load_dotenv()
             api_key = os.getenv('OPENAI_API_KEY')
             if api_key:
                 openai.api_key = api_key
        ...
        response_text = {"reply": "Invalid request. Provide genes, raw_sentences, or prompt+history.", "conversation_history": []} # Default
        try:
            input_data = web.data().decode('utf-8')
            json_data = json.loads(input_data)

            genes = json_data.get("genes") # For initial DB lookup (now less used)
            conversation_h = json_data.get("conversation_history") # For chat
            prompt = json_data.get("prompt") # For chat
            raw_sentences = json_data.get("raw_sentences") # For initial summary from list.php
            prompt_instructions = json_data.get("prompt_instructions") # Optional summary instructions
            ...
            # console.print(f"History Provided: {conversation_h is not None}")
            ...
            # Mode 3: Initial Summary from Gene Search (Fallback/Legacy - less likely needed now)
            elif genes:
                ...
                if conn and conn.is_connected():
                    ...
                    try:
                        ...
                        if sanitized_genes_tuple:
                            ...
                            query_pmid = f"SELECT DISTINCT t.PMID FROM t_sentence_hit_gene2gene_Host AS t WHERE t.geneSymbol1 IN ({placeholders}) OR t.geneSymbol2 IN ({placeholders}) LIMIT 1000" # Limit initial PMIDs
                            ...
                            if pmid_list:
                                ...
                                query_sentences = f"SELECT pmid, sentences, drug_term, hdo_term, gene_symbols FROM biosummary25_Host WHERE pmid IN ({pmid_placeholders}) LIMIT 500" # Limit sentence fetch too
...
# --- Run web.py Server using Cheroot ---
def run_webpy():
    host = "0.0.0.0"
    port = 9635
    print(f"Starting Cheroot WSGI server on http://{host}:{port}/")
    server_address = (host, port)
    # Increase threads for handling concurrent requests
    server = wsgi.Server(server_address, app.wsgifunc(), numthreads=20)
    try:
        server.start()
