...
//$strSql = "select * FROM t_sentence_hit_gene2gene_Host left join sentences24_Host on t_sentence_hit_gene2gene_Host.PMID=sentences24_Host.pmid where ";
...
if (!$rs->EOF)
{
	...
	//$strSql = "select t_sentence_hit_gene2gene_Host.*,sentences24_Host.sentence FROM t_sentence_hit_gene2gene_Host left join sentences24_Host on t_sentence_hit_gene2gene_Host.PMID=sentences24_Host.pmid where gene2gene_host_id in ('$ignets') GROUP BY sentences24_Host.sentence, t_sentence_hit_gene2gene_Host.PMID";
	$strSql = "select t_sentence_hit_gene2gene_Host.*,sentences25_genepair.sentence FROM t_sentence_hit_gene2gene_Host left join sentences25_genepair on t_sentence_hit_gene2gene_Host.PMID=sentences25_genepair.pmid where gene2gene_host_id in ('$ignets') GROUP BY sentences25_genepair.sentence, t_sentence_hit_gene2gene_Host.PMID";
...
?s <http://purl.obolibrary.org/obo/INO_0000006> ?o .
	...
	$sparql_results = my_json_query($querstring); // takes too long

        print("\nstart\n");
	echo sizeof($sparql_results);
	die();
...
?>
	<tr>
		...
		<td bgcolor="#F5FAF7" style=" font-size:12px"><?php echo htmlspecialchars($ignet['sentence'], ENT_QUOTES, 'UTF-8')?></td>
