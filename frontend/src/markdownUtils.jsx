...
// @MX:ANCHOR: [AUTO] Shared markdown renderer used by Assistant and BioSummarAI
// @MX:REASON: Fan-in >= 2 call sites; any change here affects all LLM output rendering
...
export function renderMarkdown(text) {
  ...
  return text.split('\n').map((line, i) => {
    ...
    if (numMatch) return <p key={i} className="ml-4 mb-1" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(`<span class="text-gray-400 mr-1">${numMatch[1]}.</span>${numMatch[2]}`) }} />

    // Bullet list
    if (line.startsWith('- ') || line.startsWith('* ')) return <p key={i} className="ml-4 mb-1" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(`<span class="text-gray-400 mr-1">&bull;</span>${line.slice(2)}`) }} />
