...
$params = "?c_species=$c_species&c_gene_name=$c_gene_name";
...
if (strlen($vali->getErrorMsg())==0) { 
	...
	$strSql="SELECT distinct c_mesh_UI,heading,c_hits,c_weighted_hits FROM t_gene_list, dic_mesh_desc where t_gene_list.c_mesh_UI=dic_mesh_desc.mesh_UI AND c_species = '$c_species' AND c_gene_name = '$c_gene_name'";
	...
	if (!$rs->EOF) {
		...
		$strSql="select * from t_gene_annotation where symbol ='$c_gene_name' and species = '$c_species'";
		$rs = $db->Execute($strSql);
		$synonyms = array();
		$description = array();
		if (!$rs->EOF) {
...
?>
<p><b>Gene-related MeSH Profile of <a href="http://www.phidias.us/phigen/query/gene_query_process.php?species=<?php echo $search_species?>&gene_name=<?php echo $c_gene_name?>"><?php echo $c_gene_name?></a></b></p>
<?php 
		if (!empty($synonyms)) {
?><p style="font-size:12px">Also known as: <?php echo preg_replace('/\|/', ', ', join(', ', $synonyms))?></p>
<?php 		
		}
		if (!empty($description)) {
?><p style="font-size:12px">Description: <?php echo join(', ', $description)?></p>
<?php 		
		}
?>
...
?>
  <tr>
    <td bgcolor="<?php echo $bgcolor?>" class="smallContent"><?php echo $gene_list['c_mesh_UI']?></td>
    <td bgcolor="<?php echo $bgcolor?>" class="smallContent"><?php echo $gene_list['heading']?> 
<?php 
		for($i=1; $i<=sizeof($meshuis[$gene_list['c_mesh_UI']]); $i++) {
?>
			[<a href="../meshbrowse/index.php?c_species=<?php echo $c_species?>&c_final_edge=<?php echo $meshuis[$gene_list['c_mesh_UI']][$i-1]?>"><?php echo $i?></a>] 
<?php 
		}
?>
	</td>
    <td bgcolor="<?php echo $bgcolor?>" class="smallContent"><?php echo $gene_list['c_hits']?></td>
    <td bgcolor="<?php echo $bgcolor?>" class="smallContent"><?php echo sprintf('%.6f', $gene_list['c_weighted_hits'])?></td>
  </tr>
<?php 
		}
?>
