<?php
//load TKI specific pmids into table t_pubmed_query
include('../../inc/functions.php');
$db = ADONewConnection($driver);
$db->Connect($host, $username, $password, $database);

$tki_pmids=array();
$strSql = "SELECT distinct pmid FROM ignet.sentence_oae where term_id like 'JHD_%';";
$rs = $db->Execute($strSql);
foreach($rs as $row) {
	$tki_pmids[$row['pmid']] = 1;
}
...
FILTER (?o= <$oae_id>)
	...
	foreach($rs as $row) {
		if (isset($tki_pmids[$row['pmid']])) 	$pmids[$row['pmid']] =1;
	}
...
VALUES
('$c_query_text.TKIs-specific', '$strPMIDs', '$c_query_id', 'Cardiotoxicity_oae');
";
	$db->Execute($strSql);
}
?>